...
const TableData = () => {
    const { quizSecond, fetchQuizSecond } = useAuth()
    ...
    const HandleDeleteAllQuiz = async () => {
        if (quizSecond.length > 0) {
            if (confirm('Are you sure you want to delete')) {
                try {
                    const res = await axios.delete(`${process.env.NEXT_PUBLIC_SOME_URL}/api/second-quiz`)
                    if (res.status === 200) {
    ...
    const HandleDeleteSelect = async () => {
        if (selectCheckbox.length > 0) {
            if (confirm('Are you sure you want to delete selected quiz')) {
                try {
                    const res = await axios.delete(`${process.env.NEXT_PUBLIC_SOME_URL}/api/second-quiz/selected`, { data: { ids: selectCheckbox } })
                    if (res.status === 200) {
                        toast.info(res.data.message)
                        fetchQuizSecond()
                        setSelectCheckbox([])
        ...
        <>
            <section className="main-content w-full overflow-auto p-6">
                <div className="w-full flex items-center justify-around">
                    <Link className='w-[150px] bg-blue-600 hover:bg-blue-800 text-white block text-center my-3 py-2 ' href={'/second-quiz/second-floor'}>Add User</Link>
